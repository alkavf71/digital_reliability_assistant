...
class VibrationAnalyzer:
    ...
    def check_severity(value, limit):
        """Output Remark sesuai Template Form User"""
        if value > limit: 
            return "Vibration causes damage" # Zone D
        elif value > (limit * 0.60): 
            return "Short-term operation allowable" # Zone C
        return "Unlimited long-term operation allowable" # Zone A/B
...
class StructuralAnalyzer:
    ...
    def analyze(disp_micron, vel_mm_s, limit_vel):
        # Jika Disp tinggi tapi Vel aman -> Murni Kekakuan/Looseness
        if disp_micron > 100:
            if vel_mm_s < limit_vel: return "LOOSENESS_ONLY"
            else: return "STRUCTURAL_DAMAGE"
        return "RIGID"
